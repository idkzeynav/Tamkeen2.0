...
// get all users --- admin
// get all users --- admin with search functionality
export const getAllUsers = (searchTerm = "") => async (dispatch) => {
  try {
    dispatch({
      type: "getAllUsersRequest",
    });

    // Make sure to encode the search term to handle special characters
    const encodedSearchTerm = encodeURIComponent(searchTerm);
    
    const { data } = await axios.get(`${server}/user/admin-all-users?search=${encodedSearchTerm}`, {
      withCredentials: true,
    });
